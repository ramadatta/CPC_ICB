...
ui <- fluidPage(
  ...
  # ),
  # fluidRow(column(width = 6), downloadButton("download", "Download Selected Nodes Data"),
  #          column(width=6, "Select multiple nodes by clicking and holding for a second. The datatable on the left will update accordingly.")
  # )
...
server <- function (input, output, session){
  ...
  output$download <- downloadHandler(
    filename = function() {
      paste("selected_nodes_data", Sys.Date(), ".csv", sep = "")
    },
