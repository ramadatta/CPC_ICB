...
# cd /Users/ramadatta/Analysis/Herberts_Lab/2_MHV68_Mice_Analysis/from_Lukas_HD/Datta_Redo/NN/run_3_uninfected_same_condition_results

#for days in $(ls -d */ | tr -d "/"); do cp makeLigand_TargetGene_List_v3.R $days/KO; cd $days/KO; echo $days; for d in nichenet_ligand_act*.csv; do echo -en "$d\t"; head -6 $d | tail -n+2 | awk -F, '{print $2}' | sed 's/"//g' | tr '\n' ',' | sed 's/,$//g'; echo ""; done | sed -e 's/nichenet_ligand_act_//g' -e 's/_/ /g' -e 's/.csv//g' >top5_ligands_"$days"_KO.txt; Rscript makeLigand_TargetGene_List_v3.R; cp top5_ligands* ..;  mv ../Top5_Ligands_TargetGenes.txt ../Top5_Ligands_TargetGenes_"$days".txt; cd ../..; done
...
mat1 <- read.csv(paste0('2021_01_24_CT_uninfected_CT_6d-CT_qual.csv'), row.names = 1, check.names = F, header = T)
mat1 <- as.matrix(mat1)
mat2 <- read.csv(paste0('2021_01_24_KO_uninfected_KO_6d-KO_qual.csv'), row.names = 1, check.names = F, header = T)
mat2 <- as.matrix(mat2)
...
targets_cells_top5ligands_df <- data.table::fread(paste0("Top5_Ligands_TargetGenes_",day,".txt"),sep="\t", header = TRUE) 
...
#tibble(test.visn)
#return(test.visn$nodes,test.visn$edges)

return(list(nodes = test.visn$nodes, edges = test.visn$edges, targets_cells_top5ligands_df = targets_cells_top5ligands_df))
}

# result <- process_files("2021_01_24_CT_uninfected_CT_6d-CT_qual.csv","2021_01_24_KO_uninfected_KO_6d-KO_qual.csv","6d")
# result$nodes
# result$edges
# mat1 <- read.csv(paste0('2021_01_24_CT_uninfected_CT_6d-CT_qual.csv'), row.names = 1, check.names = F, header = T)
# mat1 <- as.matrix(mat1)
